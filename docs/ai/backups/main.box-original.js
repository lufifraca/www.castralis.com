...
(() => {
    ...
    svg.innerHTML = `
      <defs>
        ...
        <radialGradient id="coreGlow" cx=".5" cy=".5" r=".5">
          ...
          <stop offset=".5" stop-color="#B5660A" stop-opacity=".35"/>
          <stop offset="1" stop-color="#B5660A" stop-opacity="0"/>
        </radialGradient>
        <linearGradient id="crystal" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stop-color="#ffe6ad"/>
          <stop offset=".45" stop-color="#B5660A"/>
      ...
      <g transform="translate(${cx} 332)" text-anchor="middle" class="node-mark">
        <text x="13" y="3" fill="#9fb0c8" font-family="'Inter',sans-serif" font-size="14" font-weight="700" letter-spacing="3.2">CASTRALIS</text>
        ...
        <rect x="-80.5" y="-4.5" width="9" height="9" rx="1" transform="rotate(45 -76 0)" fill="#B5660A"/>
      </g>
    `;
  }
  buildNode();
  ...
  function boot() {
    ...
    const lines = [
      ["> POWER-ON SELF-TEST .................. ", "PASS", "g"],
      ["> THERMAL LOOP // CLOSED-LOOP ......... ", "NOMINAL", "g"],
      ["> MESH SYNC // LIMES PROTOCOL ......... ", "LOCKED", "a"],
  ...
  function form() {
    ...
    f.addEventListener("submit", (e) => {
      ...
      status.textContent = "// SUBMITTING REQUEST…"; status.classList.remove("is-err");
      setTimeout(() => {
        btn.classList.remove("is-loading");
        $(".submit__label").textContent = "REQUEST LOGGED";
        status.textContent = "✓ REQUEST LOGGED — AWAITING INSTITUTIONAL VALIDATION";
  ...
  function buildSchematic() {
    ...
    svg.innerHTML = `
      <defs><linearGradient id="schemGem" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stop-color="#ffe6ad"/><stop offset=".5" stop-color="#B5660A"/><stop offset="1" stop-color="#7c4e12"/>
      </linearGradient></defs>
  ...
  function castrumAnimate() {
    ...
    glow.setAttribute("fill", "none"); glow.setAttribute("stroke", "#B5660A"); glow.setAttribute("stroke-width", "2");
  ...
  function initPraesidium3D() {
    ...
    const drawVent = (x, S) => {
      ...
      x.strokeStyle = "rgba(181,102,10,.28)"; x.lineWidth = S * 0.012; x.strokeRect(-g, -g, 2 * g, 2 * g); x.restore();
    };
    ...
    { const glow = new T.Sprite(new T.SpriteMaterial({ map: canvasTex(128, radial([[0,"rgba(255,235,190,.8)"],[0.4,"rgba(181,102,10,.3)"],[1,"rgba(181,102,10,0)"]]), true), blending: T.AdditiveBlending, transparent: true, depthWrite: false }));
    ...
    function wordmark() {
      ...
      x.save(); x.translate(58, 60); x.rotate(Math.PI / 4); x.strokeStyle = "#cdd8e8"; x.lineWidth = 5; x.strokeRect(-25, -25, 50, 50); x.fillStyle = "#B5660A"; x.fillRect(-12, -12, 24, 24); x.restore();
    ...
    const loop = () => {
      ...
      Object.keys(parts).forEach(k => {
        ...
        pt.edge.color.lerp(_col.setHex(k === active ? 0xB5660A : 0x6481a6), Math.min(1, dt * 4));
  ...
  function gridFx() {
    ...
    const spawn = () => {
      ...
      return { vertical, line: Math.round(Math.random() * lines) * GRID, pos: fromStart ? -40 : max + 40,
        ...
        hue: Math.random() < 0.6 ? "181,102,10" : "100,129,166" };
    };
